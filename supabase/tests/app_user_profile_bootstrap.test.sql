...
select like(
  pg_get_functiondef('core.upsert_my_app_user_profile(text,text)'::regprocedure),
...
select unlike(
  lower(pg_get_functiondef('core.upsert_my_app_user_profile(text,text)'::regprocedure)),
  '%execute %',
