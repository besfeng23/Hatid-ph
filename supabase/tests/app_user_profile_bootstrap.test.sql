...
select ok(
  position('v_actor_user_id uuid := auth.uid()' in (select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure)) > 0,
  'profile bootstrap function reads auth.uid() as actor'
);

select ok(
  position('where existing_profile.user_id = v_actor_user_id' in (select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure)) > 0,
  'profile bootstrap function targets only auth.uid() profile row'
);

select ok(
  position('execute ' in lower((select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure))) = 0,
  'profile bootstrap function does not use dynamic SQL'
);

select ok(
  position('insert into audit.audit_logs' in (select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure)) > 0,
  'profile bootstrap function writes audit log records through the controlled path'
);

select ok(
  position('core.app_users.upsert_self_profile' in (select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure)) > 0,
  'audit action name identifies self profile upsert'
);

select ok(
  position('''core.app_users''' in (select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure)) > 0,
  'audit resource type identifies core.app_users'
);

select ok(
  position('organization_members' in lower((select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure))) = 0,
  'profile bootstrap function does not write organization memberships'
);

select ok(
  position('service_role' in lower((select prosrc from pg_proc where oid = 'core.upsert_my_app_user_profile(text,text)'::regprocedure))) = 0,
